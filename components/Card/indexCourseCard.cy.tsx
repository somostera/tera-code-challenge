...
describe('<CourseCard />', () => {
  it('should render course data correctly and navigate on click', () => {
    ...
    cy.get('[data-cy=course-short_description]').should('contain.text', mockCourse.short_description);
    cy.get('[data-cy=course-category]').should('contain.text', mockCourse.category);
    cy.get('[data-cy=course-level]').should('contain.text', mockCourse.level);

    cy.get('[data-cy=course-card]')
      .should('have.attr', 'href', `/courses/${mockCourse.id}`);
  });
});
